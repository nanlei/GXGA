...
<@admin.conArea title="专项工作>>专项工作文章>>修改" id="form1">
...
<@admin.con id="datacon1">
	...
	<tr>
		...
		<td><input id="jobCategoryId" class="mini-combobox" required="true" name="jobCategoryId" style="width:150px;" value="${(article.jobCategoryId)?default('')}" textField="text" valueField="id" url="${base}/admin/jobline.do?command=ajax&ctrl=jobCategory&jobCategoryId=${(article.jobCategoryId)?default('')}" dataField="data" showNullItem="true" allowInput="false"/></td>		
	</tr>
	<tr>
...
<@admin.searchRightArea>
	<@admin.actBtn name="保存" actionName="/admin/jobline.do?command=update" event="Save"/>
</@admin.searchRightArea>
</@admin.searchArea>
</@admin.con>
</@admin.conArea>
<@admin.conArea title="页面提示" id="form2">
本页面为专项工作文章修改页面，请按照上述内容进行编辑，然后点击【保存】按钮即可。<br>
...
<@admin.script>
	...
	function Init(){
    	...
    	var jcid=jobCategoryId.getValue();
    	
    	$.ajax({
    		url: "${base}/admin/jobline.do?command=getjobheader",
    		data: { jcid : jcid},
    		type: "POST",
    		cache: false,
    ...
    function Save() {
    	...
    	$.ajax({
    		url: "${base}/admin/jobline.do?command=update",
