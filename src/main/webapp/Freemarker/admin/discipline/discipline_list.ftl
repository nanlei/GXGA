...
<@admin.script>
	...
	function Add() {
		mini.open({
			url: "/admin/discipline.do?command=createpre",
	...
	function Edit() {
		...
		if (rows.length==1) {
			mini.open({
				url: "/admin/discipline.do?command=updatedispatcher&articleId="+rows[0].articleId,
    ...
    function doDelete(rows){
    	...
    	$.ajax({
    		url: "/admin/discipline.do?command=delete",
    		data: { articleIds: ids },
