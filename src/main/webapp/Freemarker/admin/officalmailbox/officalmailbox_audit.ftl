...
<@admin.searchRightArea>	
	<@admin.actBtn name="删除" actionName="/admin/officalmailbox.do?command=auditdelete" event="Delete" icon="icon-remove"/>
</@admin.searchRightArea> 
</@admin.searchArea>
</@admin.con>
</@admin.conArea>
...
本页面为局长信箱通知单日志页面。点击加号可以看到日志变更详细信息，日志信息只能删除。<br>
</@admin.conArea>
<style type="text/css">
.detailForm td
{
...
<@admin.script>
    ...
    function doDelete(rows){
    	...
    	$.ajax({
    		url: "${base}/admin/officalmailbox.do?command=auditdelete",
    		data: { auditIds: ids },
    		success: function (data) {
    			try{
    				if(data.status=="true"){
