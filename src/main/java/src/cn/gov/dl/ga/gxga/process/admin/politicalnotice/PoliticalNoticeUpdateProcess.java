package cn.gov.dl.ga.gxga.process.admin.politicalnotice;
...
public class PoliticalNoticeUpdateProcess extends Process {
	...
	public Result process(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (StringUtils.isNotEmpty(articleDate)) {
			articleDate = new SimpleDateFormat("yyyy-MM-dd")
					.format(new SimpleDateFormat("yyyy-MM-dd").parse(articleDate));
		} else {
			articleDate = null;
		}

		Object[] parameters = new Object[] { articleTitle, articleContent, articleOrder, articleDate, createBy,
				createByName, createByIP, articleId };
