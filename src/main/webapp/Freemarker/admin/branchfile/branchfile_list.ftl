<@admin.page title="分局文件">
<@admin.conArea title="前台综合>>分局文件>>查询" id="form1">
...
<div id="datagrid1" class="mini-datagrid" style="width:100%;height:100%;padding:0;margin:0;" allowResize="true"
    url="${base}/admin/branchfile.do?command=search"  idField="articleId" multiSelect="true">
...
<@admin.script>
	...
	function Add() {
		mini.open({
			url: "/admin/branchfile.do?command=createpre",
	...
	function Edit() {
		...
		if (rows.length==1) {
			mini.open({
				url: "/admin/branchfile.do?command=updatedispatcher&articleId="+rows[0].articleId,
    ...
    function doDelete(rows){
    	...
    	$.ajax({
    		url: "/admin/branchfile.do?command=delete",
    		data: { articleIds: ids },
