...
<@admin.conArea title="网上办公>>采稿排名>>查询" id="form1">
...
<@admin.searchLeftArea>	
	<@admin.actBtn name="查询" actionName="/admin/rank.do?command=search" event="Search" icon="icon-search"/>
</@admin.searchLeftArea>  
<@admin.searchRightArea>	
	<@admin.actBtn name="新建" actionName="/admin/rank.do?command=createpre" event="Add" icon="icon-add"/>
	<@admin.actBtn name="修改" actionName="/admin/rank.do?command=updatepre" event="Edit" icon="icon-edit"/>
	<@admin.actBtn name="删除" actionName="/admin/rank.do?command=delete" event="Delete" icon="icon-remove"/>
</@admin.searchRightArea> 
</@admin.searchArea>
</@admin.con>
</@admin.conArea>
...
<div id="datagrid1" class="mini-datagrid" style="width:100%;height:100%;padding:0;margin:0;" allowResize="true"
    url="${base}/admin/rank.do?command=search"  idField="rankId" multiSelect="true">
    <div property="columns">
        <div type="checkcolumn"></div>
        <div type="indexcolumn"headerAlign="center">序号</div>
        <div field="departmentName" width="70" headerAlign="center" align="center" allowSort="false">所属部门</div>
        <div field="rankValue" width="60" headerAlign="center" align="center" allowSort="true">采稿数量</div>
        <div field="rankOrder" width="60" headerAlign="center" align="center" allowSort="true">排序</div>
...
<@admin.script>
	...
	function Add() {
		mini.open({
			url: "${base}/admin/rank.do?command=createpre",
	...
	function Edit() {
		...
		if (rows.length==1) {
			mini.open({
				url: "${base}/admin/rank.do?command=updatepre&rankId="+rows[0].rankId,
				title: "修改采稿排名："+rows[0].departmentName, width: 400, height: 300,
				ondestroy: function (action) {
					Search();
                }
            });
    ...
    function doDelete(rows){
    	...
    	$.ajax({
    		url: "${base}/admin/rank.do?command=delete",
    		data: { rankIds: ids },
