<@admin.page title="人才市场">
<@admin.conArea title="人力资源>>人才市场>>查询" id="form1">
<@admin.con id="datacon1">
	<tr>
		<td>标题:</td>
		<td><input class="mini-textbox" required="false" name="articleTitle"  style="width:150px;"/></td>
		<td>状态:</td>
		<td><input id="articleStatus" class="mini-combobox" required="false" name="articleStatus" style="width:150px;" textField="text" valueField="id"  url="${base}/admin/const.do?constant=ARTICLESTATUS" dataField="data" showNullItem="true" allowInput="false" /></td>
		<td>所属部门:</td>
	...
	<tr>
		<td>类型:</td>
		<td><input id="articleCode" class="mini-combobox" required="false" name="articleCode" style="width:150px;" textField="text" valueField="id"  url="${base}/admin/const.do?constant=TALENT" dataField="data" showNullItem="true" allowInput="false" /></td>
	</tr>
<@admin.searchArea colspan="8">
<@admin.searchLeftArea>	
	<@admin.actBtn name="查询" actionName="/admin/talent.do?command=search" event="Search" icon="icon-search"/>
</@admin.searchLeftArea>  
<@admin.searchRightArea>	
	<@admin.actBtn name="新建" actionName="/admin/talent.do?command=createpre" event="Add" icon="icon-add"/>
	<@admin.actBtn name="修改" actionName="/admin/talent.do?command=updatedispatcher" event="Edit" icon="icon-edit"/>
	<@admin.actBtn name="删除" actionName="/admin/talent.do?command=delete" event="Delete" icon="icon-remove"/>
</@admin.searchRightArea> 
</@admin.searchArea>
</@admin.con>
</@admin.conArea>
...
<div id="datagrid1" class="mini-datagrid" style="width:100%;height:100%;padding:0;margin:0;" allowResize="true"
    url="${base}/admin/talent.do?command=search"  idField="articleId" multiSelect="true">
    <div property="columns">
        ...
        <div field="articleCode" width="60" headerAlign="center" align="center" allowSort="true">类型</div>
...
<@admin.script>
	...
	function Add() {
		mini.open({
			url: "/admin/talent.do?command=createpre",
			title: "新建人才市场", width: 900, height: 580,
	...
	function Edit() {
		...
		if (rows.length==1) {
			mini.open({
				url: "/admin/talent.do?command=updatedispatcher&articleId="+rows[0].articleId,
				title: "修改人才市场："+rows[0].articleTitle, width: 900, height: 610,
				ondestroy: function (action) {
					Search();
                }
            });
    ...
    function doDelete(rows){
    	...
    	$.ajax({
    		url: "/admin/talent.do?command=delete",
    		data: { articleIds: ids },
