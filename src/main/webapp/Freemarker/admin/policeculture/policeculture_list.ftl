...
<@admin.script>
	...
	function Add() {
		mini.open({
			url: "/admin/policeculture.do?command=createpre",
	...
	function Edit() {
		...
		if (rows.length==1) {
			mini.open({
				url: "/admin/policeculture.do?command=updatedispatcher&articleId="+rows[0].articleId,
    ...
    function doDelete(rows){
    	...
    	$.ajax({
    		url: "/admin/policeculture.do?command=delete",
    		data: { articleIds: ids },
