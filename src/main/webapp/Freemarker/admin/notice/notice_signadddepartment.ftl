<@admin.page title="通知通报添加签收部门">
<@admin.conArea title="前台综合>>通知通报>>添加签收部门" id="form1">
<input class="mini-hidden" name="articleId" value="${(articleId)?default('')}"/>
<@admin.con id="datacon1">
	<tr>
		<td><label>部门:</label></td>
		<td><input class="mini-treeselect" id="departmentId" required="false" name="departmentId" style="width:140px;" textField="text" valueField="id" parentField="upperId" dataField="data" url="/admin/department.do?command=getupperdepartment" showNullItem="true" allowInput="true" popupWidth="150px" showFolderCheckBox="true" showClose="true" oncloseclick="onCloseClick" showTreeIcon="false" onValueChanged="onDepartmentChanged"/></td>
	</tr>
	<tr id="leader">
...
<@admin.searchRightArea>
	<@admin.actBtn name="保存" actionName="/admin/notice.do?command=signadddepartment" event="Save"/>
</@admin.searchRightArea>
</@admin.searchArea>
</@admin.con>
</@admin.conArea>
<@admin.conArea title="页面提示" id="form2">
本页面为通知通报添加签收部门页面，请选择部门，然后点击【保存】按钮即可。
</@admin.conArea>
<@admin.script>
	mini.parse();
    ...
    function Save() {
    	...
    	$.ajax({
    		url: "/admin/notice.do?command=signadddepartment",
    		data: { object: json},
    		type: "POST",
    		cache: false,
