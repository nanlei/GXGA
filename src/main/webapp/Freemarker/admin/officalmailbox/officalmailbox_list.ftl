...
<@admin.searchLeftArea>	
	<@admin.actBtn name="查询" actionName="/admin/officalmailbox.do?command=search" event="Search" icon="icon-search"/>
</@admin.searchLeftArea>  
<@admin.searchRightArea>	
	<@admin.actBtn name="查看" actionName="/admin/officalmailbox.do?command=detaildispatcher" event="Detail" icon="icon-find"/>
	<@admin.actBtn name="新建" actionName="/admin/officalmailbox.do?command=createpre" event="Add" icon="icon-add"/>
	<@admin.actBtn name="修改" actionName="/admin/officalmailbox.do?command=updatedispatcher" event="Edit" icon="icon-edit"/>
	<@admin.actBtn name="删除" actionName="/admin/officalmailbox.do?command=delete" event="Delete" icon="icon-remove"/>
</@admin.searchRightArea> 
</@admin.searchArea>
</@admin.con>
</@admin.conArea>
...
<div id="datagrid1" class="mini-datagrid" style="width:100%;height:100%;padding:0;margin:0;" allowResize="true"
    url="/admin/officalmailbox.do?command=search"  idField="transactionId" multiSelect="true">
...
<@admin.script>
	...
	function Detail(){
		...
		if (rows.length==1) {
			mini.open({
				url: "${base}/admin/officalmailbox.do?command=detaildispatcher&transactionId="+rows[0].transactionId,
				title: "查看局长信箱通知单："+rows[0].transactionTitle+" ("+rows[0].transactionNo+")", width: 900, height: 600,
	...
	function Add() {
		mini.open({
			url: "${base}/admin/officalmailbox.do?command=createpre",
			title: "新建局长信箱通知单", width: 900, height: 600,
			ondestroy: function (action) {
				Search();
			}
		});
	...
	function Edit() {
		...
		if (rows.length==1) {
			mini.open({
				url: "${base}/admin/officalmailbox.do?command=updatedispatcher&transactionId="+rows[0].transactionId,
				title: "修改局长信箱通知单："+rows[0].transactionTitle+" ("+rows[0].transactionNo+")", width: 900, height: 600,
				ondestroy: function (action) {
					Search();
                }
            });
    ...
    function doDelete(rows){
    	...
    	$.ajax({
    		url: "${base}/admin/officalmailbox.do?command=delete",
