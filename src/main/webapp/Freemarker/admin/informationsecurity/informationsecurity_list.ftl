...
<@admin.script>
	...
	function Add() {
		mini.open({
			url: "/admin/informationsecurity.do?command=createpre",
			title: "新建信息安全", width: 900, height: 580,
	...
	function Edit() {
		...
		if (rows.length==1) {
			mini.open({
				url: "/admin/informationsecurity.do?command=updatedispatcher&articleId="+rows[0].articleId,
    ...
    function doDelete(rows){
    	...
    	$.ajax({
    		url: "/admin/informationsecurity.do?command=delete",
    		data: { articleIds: ids },
