<@admin.page title="纪检监察">
<@admin.conArea title="前台综合>>纪检监察>>查询" id="form1">
...
<@admin.searchLeftArea>	
	<@admin.actBtn name="查询" actionName="/admin/discipline.do?command=search" event="Search" icon="icon-search"/>
</@admin.searchLeftArea>  
<@admin.searchRightArea>	
	<@admin.actBtn name="新建" actionName="/admin/discipline.do?command=createpre" event="Add" icon="icon-add"/>
	<@admin.actBtn name="修改" actionName="/admin/discipline.do?command=updatedispatcher" event="Edit" icon="icon-edit"/>
	<@admin.actBtn name="删除" actionName="/admin/discipline.do?command=delete" event="Delete" icon="icon-remove"/>
</@admin.searchRightArea> 
</@admin.searchArea>
</@admin.con>
</@admin.conArea>
...
<div id="datagrid1" class="mini-datagrid" style="width:100%;height:100%;padding:0;margin:0;" allowResize="true"
    url="${base}/admin/discipline.do?command=search"  idField="articleId" multiSelect="true">
...
<@admin.script>
	...
	function Add() {
		mini.open({
			url: "${base}/admin/discipline.do?command=createpre",
			title: "新建纪检监察", width: 900, height: 580,
	...
	function Edit() {
		...
		if (rows.length==1) {
			mini.open({
				url: "${base}/admin/discipline.do?command=updatedispatcher&articleId="+rows[0].articleId,
				title: "修改纪检监察："+rows[0].articleTitle, width: 900, height: 610,
				ondestroy: function (action) {
					Search();
                }
            });
    ...
    function doDelete(rows){
    	...
    	$.ajax({
    		url: "${base}/admin/discipline.do?command=delete",
    		data: { articleIds: ids },
