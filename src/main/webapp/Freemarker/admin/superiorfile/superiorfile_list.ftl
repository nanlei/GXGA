<@admin.page title="上级文件">
<@admin.conArea title="前台综合>>上级文件>>查询" id="form1">
...
<@admin.searchLeftArea>	
	<@admin.actBtn name="查询" actionName="/admin/superiorfile.do?command=search" event="Search" icon="icon-search"/>
</@admin.searchLeftArea>  
<@admin.searchRightArea>	
	<@admin.actBtn name="新建" actionName="/admin/superiorfile.do?command=createpre" event="Add" icon="icon-add"/>
	<@admin.actBtn name="修改" actionName="/admin/superiorfile.do?command=updatedispatcher" event="Edit" icon="icon-edit"/>
	<@admin.actBtn name="签收" actionName="/admin/superiorfile.do?command=signpre" event="Sign" icon="icon-ok"/>
	<@admin.actBtn name="删除" actionName="/admin/superiorfile.do?command=delete" event="Delete" icon="icon-remove"/>
</@admin.searchRightArea> 
</@admin.searchArea>
</@admin.con>
</@admin.conArea>
...
<div id="datagrid1" class="mini-datagrid" style="width:100%;height:100%;padding:0;margin:0;" allowResize="true"
    url="${base}/admin/superiorfile.do?command=search"  idField="articleId" multiSelect="true">
...
<@admin.script>
	...
	function Add() {
		mini.open({
			url: "${base}/admin/superiorfile.do?command=createpre",
			title: "新建上级文件", width: 900, height: 600,
	...
	function Edit() {
		...
		if (rows.length==1) {
			mini.open({
				url: "${base}/admin/superiorfile.do?command=updatedispatcher&articleId="+rows[0].articleId,
    ...
    function doDelete(rows){
    	...
    	$.ajax({
    		url: "${base}/admin/superiorfile.do?command=delete",
    		data: { articleIds: ids },
