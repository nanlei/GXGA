<@admin.page title="信息安全">
<@admin.conArea title="前台综合>>信息安全>>查询" id="form1">
...
<@admin.searchLeftArea>	
	<@admin.actBtn name="查询" actionName="/admin/informationsecurity.do?command=search" event="Search" icon="icon-search"/>
</@admin.searchLeftArea>  
<@admin.searchRightArea>	
	<@admin.actBtn name="新建" actionName="/admin/informationsecurity.do?command=createpre" event="Add" icon="icon-add"/>
	<@admin.actBtn name="修改" actionName="/admin/informationsecurity.do?command=updatedispatcher" event="Edit" icon="icon-edit"/>
	<@admin.actBtn name="删除" actionName="/admin/informationsecurity.do?command=delete" event="Delete" icon="icon-remove"/>
</@admin.searchRightArea> 
</@admin.searchArea>
</@admin.con>
</@admin.conArea>
...
<div id="datagrid1" class="mini-datagrid" style="width:100%;height:100%;padding:0;margin:0;" allowResize="true"
    url="/admin/informationsecurity.do?command=search"  idField="articleId" multiSelect="true">
...
<@admin.script>
	...
	function Add() {
		mini.open({
			url: "${base}/admin/informationsecurity.do?command=createpre",
			title: "新建信息安全", width: 900, height: 600,
	...
	function Edit() {
		...
		if (rows.length==1) {
			mini.open({
				url: "${base}/admin/informationsecurity.do?command=updatedispatcher&articleId="+rows[0].articleId,
				title: "修改信息安全："+rows[0].articleTitle, width: 900, height: 610,
				ondestroy: function (action) {
					Search();
                }
            });
    ...
    function doDelete(rows){
    	...
    	$.ajax({
    		url: "${base}/admin/informationsecurity.do?command=delete",
    		data: { articleIds: ids },
