...
<@admin.searchLeftArea>	
	<@admin.actBtn name="查询" actionName="/admin/evaluation.do?command=search" event="Search" icon="icon-search"/>
</@admin.searchLeftArea>  
<@admin.searchRightArea>	
	<@admin.actBtn name="新建" actionName="/admin/evaluation.do?command=createpre" event="Add" icon="icon-add"/>
	<@admin.actBtn name="修改" actionName="/admin/evaluation.do?command=updatedispatcher" event="Edit" icon="icon-edit"/>
	<@admin.actBtn name="删除" actionName="/admin/evaluation.do?command=delete" event="Delete" icon="icon-remove"/>
</@admin.searchRightArea> 
</@admin.searchArea>
</@admin.con>
</@admin.conArea>
...
<div id="datagrid1" class="mini-datagrid" style="width:100%;height:100%;padding:0;margin:0;" allowResize="true"
    url="${base}/admin/evaluation.do?command=search"  idField="articleId" multiSelect="true">
...
<@admin.script>
	...
	function Add() {
		mini.open({
			url: "/admin/evaluation.do?command=createpre",
	...
	function Edit() {
		...
		if (rows.length==1) {
			mini.open({
				url: "/admin/evaluation.do?command=updatedispatcher&articleId="+rows[0].articleId,
    ...
    function doDelete(rows){
    	...
    	$.ajax({
    		url: "/admin/evaluation.do?command=delete",
    		data: { articleIds: ids },
