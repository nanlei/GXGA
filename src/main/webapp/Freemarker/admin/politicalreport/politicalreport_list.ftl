...
<@admin.script>
	...
	function Add() {
		mini.open({
			url: "/admin/politicalreport.do?command=createpre",
			title: "新建政工简报", width: 900, height: 580,
	...
	function Edit() {
		...
		if (rows.length==1) {
			mini.open({
				url: "/admin/politicalreport.do?command=updatedispatcher&articleId="+rows[0].articleId,
    ...
    function doDelete(rows){
    	...
    	$.ajax({
    		url: "/admin/politicalreport.do?command=delete",
    		data: { articleIds: ids },
