...
<@admin.script>
	...
	function Add() {
		mini.open({
			url: "/admin/law.do?command=createpre",
			title: "新建法律", width: 900, height: 580,
	...
	function Edit() {
		...
		if (rows.length==1) {
			mini.open({
				url: "/admin/law.do?command=updatedispatcher&articleId="+rows[0].articleId,
    ...
    function doDelete(rows){
    	...
    	$.ajax({
    		url: "/admin/law.do?command=delete",
    		data: { articleIds: ids },
