...
public class MinYiWangAudit {
	...
	public MinYiWangAudit() {
		super();
	}

	public MinYiWangAudit(HashMap<String, Object> transaction,
	...
	public void auditTractionConfig(List<Audit> auditConfig) {
		...
		Field[] fields = MinYiWang.class.getDeclaredFields();
		...
		for (int i = 0; i < fields.length; i++) {
			...
			} else if ("config_noticeContent".equals(fieldName)) {
				if (!StringUtils.equals(dbTransactionConfigMap.get(fieldName),
						transactionConfigMap.get(fieldName))) {
					auditConfig.add(new Audit(fieldName, "内容有变动", "内容有变动"));
				}
