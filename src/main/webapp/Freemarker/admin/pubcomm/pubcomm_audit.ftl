...
<@admin.conArea title="网上办公>>公众交流平台>>日志>>查询" id="form1">
...
<@admin.searchLeftArea>	
	<@admin.actBtn name="查询" actionName="/admin/pubcomm.do?command=auditsearch" event="Search" icon="icon-search"/>
</@admin.searchLeftArea>  
<@admin.searchRightArea>	
	<@admin.actBtn name="删除" actionName="/admin/pubcomm.do?command=auditdelete" event="Delete" icon="icon-remove"/>
</@admin.searchRightArea> 
</@admin.searchArea>
</@admin.con>
</@admin.conArea>
...
<div id="datagrid1" class="mini-datagrid" style="width:100%;height:100%;padding:0;margin:0;" allowResize="true"
    url="${base}/admin/pubcomm.do?command=auditsearch"  idField="auditId" multiSelect="true" 
    <#if ftlUtil.hasPermission('/admin/pubcomm.do?command=auditdetail','${loginUser.roleId}')>
...
本页面为公众交流平台回执单日志页面。点击加号可以看到日志变更详细信息，日志信息只能删除。<br>
</@admin.conArea>
<style type="text/css">
.detailForm td
{
...
<@admin.script>
	...
	function onShowRowDetail(e){
        ...
        $.ajax({
            url: "${base}/admin/pubcomm.do?command=auditdetail&auditId="+row.auditId,
    ...
    function doDelete(rows){
    	...
    	$.ajax({
    		url: "${base}/admin/pubcomm.do?command=auditdelete",
    		data: { auditIds: ids },
    		success: function (data) {
    			try{
    				if(data.status=="true"){
