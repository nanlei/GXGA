...
<#if loginUser.departmentId==0>
	<tr>
		<td>部门:</td>
		<td><input class="mini-treeselect" id="departmentId" required="true" name="departmentId" value="1" onValueChanged="onDepartmentChanged" style="width:150px;" textField="text" valueField="id" parentField="upperId" dataField="data" url="${base}/admin/department.do?command=getupperdepartment" showNullItem="true" allowInput="false" popupWidth="150px" showFolderCheckBox="true" showClose="true" oncloseclick="onCloseClick" showTreeIcon="false"/></td>
	</tr>
	<tr>
		<td>分类：</td>
		<td><input class="mini-combobox" id="dcId" required="true" name="dcId" style="width:150px;" textField="text" valueField="id" dataField="data" url="${base}/admin/departmentcategory.do?command=getcategorybydepartment" showNullItem="true" allowInput="false" /></td>
	</tr>
<#else>
<input class="mini-hidden" name="departmentId" value="${(loginUser.departmentId)?default('')}" />
	<tr>
		<td>分类：</td>
		<td><input class="mini-combobox" id="dcId" required="true" name="dcId" style="width:150px;" textField="text" valueField="id" dataField="data" url="${base}/admin/departmentcategory.do?command=getcategorybydepartment&departmentId=${loginUser.departmentId}" showNullItem="true" allowInput="false" /></td>
	</tr>
</#if>
	<tr>
	...
	<tr>
		<td colspan="2"><@admin.ckeditor id="articleContent" name="articleContent" value="请输入内容（部门综合）" /></td>
	</tr>
<@admin.searchArea colspan="2">
<@admin.searchRightArea>
	<@admin.actBtn name="保存" actionName="/admin/branchdepartment.do?command=create" event="Save"/>
</@admin.searchRightArea>
</@admin.searchArea>
</@admin.con>
</@admin.conArea>
...
<@admin.script>
    ...
    function Save() {
    	...
    	$.ajax({
    		url: "${base}/admin/branchdepartment.do?command=create",
