...
<@admin.conArea title="前台综合>>上级文件>>签收明细" id="form1">
<input class="mini-hidden" name="articleId" value="${(articleId)?default('')}"/>
<@admin.con id="datacon1">
	<tr>
		<td>状态:</td>
		<td><input id="departmentId" class="mini-combobox" required="false" name="status" style="width:150px;" textField="text" valueField="id"  url="${base}/admin/const.do?constant=SIGNSTATUS" dataField="data" showNullItem="true" allowInput="true" /></td>
	</tr>
<@admin.searchArea colspan="8">
<@admin.searchLeftArea>
	<@admin.actBtn name="查询" actionName="/admin/superiorfile.do?command=signsearch" event="Search" icon="icon-search"/>
	<@admin.actBtn name="添加部门" actionName="/admin/superiorfile.do?command=signadddepartmentpre" event="Add" icon="icon-add"/>
	<@admin.actBtn name="移除部门" actionName="/admin/superiorfile.do?command=signdeletedepartment" event="Delete" icon="icon-remove"/>
</@admin.searchLeftArea>  
</@admin.searchArea>
</@admin.con>
</@admin.conArea>
...
<div id="datagrid1" class="mini-datagrid" style="width:100%;height:100%;padding:0;margin:0;" allowResize="true"
    url="${base}/admin/superiorfile.do?command=signsearch"  idField="asId" multiSelect="true">
...
<@admin.script>
	...
	function Add() {
		mini.open({
			url: "${base}/admin/superiorfile.do?command=signadddepartmentpre&articleId=${articleId}",
    ...
    function doDelete(rows){
    	...
    	$.ajax({
    		url: "${base}/admin/superiorfile.do?command=signdeletedepartment",
